...
export class aplicacionController {
    ...
    static async guardar (req, res) {
        ...
        if (req.session.tipo_proyecto == "1") {
            ...
            try {
                console.log(req.session.id_proyecto, req.session.id_usuario)
                const imagenes = await uploadAsync(req.body.cantidad_imagenes);
                ...
                console.log("ASDASLFJHASDFKAJSD")
                ...
                if (proyectos.length == 0) {
                    ...
                    if (imagenes.length == 0) {
                        ...
                        const [resultadoImagenes] = await connection.query('SELECT * FROM proyecto WHERE id_proyecto = ? AND id_usuario = ?', [req.session.id_proyecto, req.session.id_usuario])
                            ...
                            try {
                                ...
                                try {
                                    ...
                                    imagenesParaEliminar.forEach(nombreImagen => {
                                        const rutaImagen = path.join(process.cwd(), 'imagenes', nombreImagen);
        ...
        if (req.session.nombre_proyecto == '' || req.session.nombre_proyecto === undefined) {
            ...
            try {
                ...
                const imagenesBase64 = imagenes.map(imagen => {
                    const filePath = path.join(process.cwd(), 'imagenes', imagen.nombre);
